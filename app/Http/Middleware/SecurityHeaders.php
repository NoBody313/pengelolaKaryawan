...
class SecurityHeaders
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!app()->environment('local')) {
            ...
            $response->headers->set('Content-Security-Policy', "default-src 'self'; script-src 'self' platform.twitter.com plausible.io utteranc.es *.cloudflare.com 'unsafe-inline' 'unsafe-eval' plausible.io/js/plausible.js utteranc.es/client.js hcaptcha.com; style-src 'self' *.cloudflare.com 'unsafe-inline' fonts.googleapis.com; img-src 'self' * data:; font-src 'self' data: fonts.gstatic.com; connect-src 'self' plausible.io/api/event hcaptcha.com; media-src 'self'; frame-src 'self'; object-src 'none'; base-uri 'self';");
